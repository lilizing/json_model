    ...
    if (FileSystemEntity.isFileSync(f.path)) {
      ...
      (map as Map<String, dynamic>).forEach((key, v) {
        ...
        if (key.startsWith("@")) {
          ...
          var value = getType(v, set, name, tag);
          attrs.write(value);
          attrs.write(" ");
          attrs.write(key);
          attrs.writeln(";");
      ...
      //下划线转驼峰
      String className = name.replaceAllMapped(RegExp(r'_(\w)'), (Match m) {
  ...
  if (v is bool) {
    ...
    if (v.startsWith("$tag[]")) {
      ...
      if (type.toLowerCase() != current && !isBuiltInType(type)) {
        ...
        var imp = type.replaceAllMapped(RegExp(r'([A-Z])'), (Match m){
          return '_' + m.group(1).toLowerCase();
        });

        set.add('import "$imp.dart"');
      }
      ...
      if (fileName.toLowerCase() != current) {
        ...
        var imp = fileName.replaceAllMapped(RegExp(r'([A-Z])'), (Match m){
          return '_' + m.group(1).toLowerCase();
        });
